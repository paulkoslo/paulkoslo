<div align="center">

# Hey, I'm Paul 👋

### Builder · Data Enthusiast · Surfer 

I like turning ideas into software.
Always exploring, always shipping.

<br>

[![Website](https://img.shields.io/badge/🌐_koslowsky.net-000?style=for-the-badge&logo=safari&logoColor=white)](https://www.koslowsky.net)

</div>

---

## 🚀 Featured Projects

<table>
<tr>
<td width="33%" valign="top">

### 🗂️ [Fily](https://github.com/paulkoslo/Fily)

> *Your files, intelligently organized — without moving a thing.*

A **local-first macOS app** built with TypeScript that indexes every file on your machine and uses AI to build a beautifully organized virtual folder tree. No renaming, no moving originals — just smart, agentic file management that works the way your brain does.

`TypeScript` · `AI-Powered` · `macOS` · `Local-First`

[![Fily](https://github-readme-stats.vercel.app/api/pin/?username=paulkoslo&repo=Fily&theme=vue&hide_border=true)](https://github.com/paulkoslo/Fily)

</td>
<td width="33%" valign="top">

### 🇪🇺 [EUROWATCH](https://github.com/paulkoslo/EUROWATCH)

> *Making European Parliament debates accessible and analyzable.*

A full-stack **data platform** that scrapes, collects, and analyzes speeches from the European Parliament. Powered by AI topic classification, multi-language detection, and interactive analytics dashboards — putting political transparency at your fingertips.

`JavaScript` · `Data Science` · `NLP` · `Politics`

[![EUROWATCH](https://github-readme-stats.vercel.app/api/pin/?username=paulkoslo&repo=EUROWATCH&theme=vue&hide_border=true)](https://github.com/paulkoslo/EUROWATCH)

</td>
<td width="33%" valign="top">

### ⚡ [Enhancer](https://github.com/paulkoslo/Enhancer)

> *Supercharge your spreadsheets with AI-driven research.*

An **agentic Python system** that takes your Excel documents and automatically enriches them with AI-powered research data. Point it at a spreadsheet, tell it what you need, and watch it fill in the blanks — turning static tables into living, data-rich assets.

`Python` · `Agentic AI` · `Excel` · `Research Automation`

[![Enhancer](https://github-readme-stats.vercel.app/api/pin/?username=paulkoslo&repo=Enhancer&theme=vue&hide_border=true)](https://github.com/paulkoslo/Enhancer)

</td>
</tr>
</table>

---

## 📊 Activity

[![Ashutosh's github activity graph](https://github-readme-activity-graph.vercel.app/graph?username=paulkoslo&theme=github-light)](https://github.com/ashutosh00710/github-readme-activity-graph)

---

<div align="center">

</div>